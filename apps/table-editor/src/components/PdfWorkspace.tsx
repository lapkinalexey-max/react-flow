...
// 👇 ИМПОРТИРУЕМ НОВЫЕ СЕРВИСЫ (Убедись, что эти файлы созданы!)
import { cropImage } from "../utils/imageCropper";
import { recognizeTableWithAi } from "../services/AiTableService";
...
const Icons = {
  ...
  Import: () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>,
  Key: () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"></path></svg>
};
...
type SelectionRect = { startX: number; startY: number; width: number; height: number; };
...
export default function PdfWorkspace({ page, onClose, onImport }: Props) {
  ...
  const [isProcessing, setIsProcessing] = useState(false); // Индикатор загрузки AI
  ...
  const handleProcessTable = async () => {
    ...
    if (!apiKey) {
      alert("Пожалуйста, введите OpenAI API Key в верхнем меню!");
      return;
    }
    ...
    try {
      // 1. Вырезаем картинку
      const base64Image = await cropImage(page.imageUrl, rect);

      // 2. Отправляем в AI
      const tableData = await recognizeTableWithAi(apiKey, base64Image);
  ...
  const headerBtnStyle: React.CSSProperties = { display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", borderRadius: 6, fontSize: 14, fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap", minWidth: "max-content", flexShrink: 0 };
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100vh", background: "#111" }}>
      ...
      {/* 1. HEADER */}
      <div style={{ height: 60, background: "#252526", borderBottom: "1px solid #333", display: "flex", alignItems: "center", padding: "0 20px", justifyContent: "space-between", zIndex: 50, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {/* Input для API Key */}
          <div style={{ display: "flex", alignItems: "center", background: "#111", border: "1px solid #444", borderRadius: 6, padding: "0 8px" }}>
            <Icons.Key />
            <input
              type="password"
              ...
              style={{ background: "transparent", border: "none", color: "#fff", padding: "8px", width: 120, outline: "none", fontSize: 12 }}
            />
          </div>
          <span style={{ color: "#777", fontSize: 13, borderLeft: "1px solid #444", paddingLeft: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          {selection && (
            <button
              ...
              style={{ ...headerBtnStyle, background: isProcessing ? "#555" : "#2e8", color: isProcessing ? "#ccc" : "#000", border: "none" }}
            >
              {isProcessing ? "Думаю..." : <><Icons.Check /> Распознать AI</>}
            </button>
          )}
          <button onClick={onClose} style={{ ...headerBtnStyle, background: "#333", color: "#ddd", border: "1px solid #555" }}><Icons.Close /> Закрыть</button>
        </div>
      </div>
        ...
        <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 40 }}>
          <div style={{ background: "#1e1e1e", border: "1px solid #444", borderRadius: 12, display: "flex", flexDirection: "column", maxWidth: "90%", maxHeight: "90%", boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}>
            <div style={{ padding: "16px 24px", borderBottom: "1px solid #333", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ margin: 0, color: "#fff" }}>AI Результат</h3>
            ...
            <div style={{ padding: "16px 24px", borderTop: "1px solid #333", display: "flex", justifyContent: "flex-end", gap: 12, background: "#252526", borderBottomLeftRadius: 12, borderBottomRightRadius: 12 }}>
              ...
              <button onClick={handleImport} style={{ padding: "10px 20px", background: "#2e8", color: "#000", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: "bold", display: "flex", alignItems: "center", gap: 8 }}>
                <Icons.Import /> Добавить в граф
              </button>
            </div>
          </div>
        </div>
